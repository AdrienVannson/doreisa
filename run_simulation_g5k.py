...
head_node_cmd = execo.SshProcess(
    'singularity exec doreisa/docker/images/doreisa-analytics.sif bash -c "ray start --head --port=4242; python3 doreisa/head.py"',
    head_node,
)
head_node_cmd.start()
...
cmd_simulation = execo.SshProcess(
    f"""bash -c "cd doreisa && singularity exec docker/images/doreisa-simulation.sif mpirun -n 4 --hostfile hostfile singularity exec docker/images/doreisa-simulation.sif ./simulation" """,
    head_node,
)
